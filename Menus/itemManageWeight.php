...
<body>
    ...
    <div class="container mt-3">
        ...
        <div class="row">
            <div class="col">
                <h5>&lt;<a href="weightMenu.php?input=+-&search=Search">Back to Weight Search Page</a></h5>
            </div>
            <div class="col align-middle text-end align-self-center">
                <?php if (!empty($product)) { ?>
                <a href="itemManage.php" class="btn btn-primary">+ Create NEW Product</a>
                <?php } ?>
            </div>
        ...
        <div class="row">
            ...
            <div class="col">
                ...
                <form action="itemManageWeight.php" method="post">
                    <input value="<?= $product["weightName"] ?? "" ?>" placeholder="Weight Name" id="weightName"
                        name="weightName" class="form-control" />
...
function createProduct() {
    ...
    if(isset($stmt)) { 
        ...
        if ($stmt->execute() === TRUE) {
            $GLOBALS["weightName"] = $stmt->insert_id; // this returns the ID of the product that was JUST inserted
...
//READ
function getProduct($weightName) {
    $rValue = array();
    $sql = "SELECT 
                *
            FROM
...
function deleteProduct() {
    $sql = "DELETE FROM 
            ...
            WHERE 
                weightName=?";
    $stmt = $GLOBALS["conn"]->prepare($sql);
    $stmt->bind_param("s", $_POST["weightName"]);
    $stmt->execute();
