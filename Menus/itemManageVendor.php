...
<body>
    ...
    <div class="container mt-3">
        ...
        <div class="row">
            <div class="col">
                <h5>&lt;<a href="vendorMenu.php?input=+-&search=Search">Back to Vendor Search Page</a></h5>
            </div>
            <div class="col align-middle text-end align-self-center">
                <?php if (!empty($product)) { ?>
                <a href="itemManageVendor.php" class="btn btn-primary">+ Create NEW Product</a>
                <?php } ?>
            </div>
        ...
        <div class="row">
            ...
            <div class="col">
                ...
                <form action="itemManageVendor.php" method="post">
                    <input value="<?= $product["vendorName"] ?? "" ?>" placeholder="Vendor Name" id="vendorName"
                        name="vendorName" class="form-control" />

                    <!-- BUTTONS -->
                    <div class="text-center">
                        <?php if (empty($product)) { ?>
                        <a href="vendorMenu.php?input=+-&search=Search" class="btn btn-warning mt-2">Cancel</a>
...
function createProduct() {
    ...
    if(isset($stmt)) { 
        ...
        if ($stmt->execute() === TRUE) {
            $GLOBALS["vendorName"] = $stmt->insert_id; // this returns the ID of the product that was JUST inserted
...
//READ
function getProduct($vendorName) {
    $rValue = array();
    $sql = "SELECT 
                *
            FROM
...
function deleteProduct() {
    $sql = "DELETE FROM 
            ...
            WHERE 
                vendorName=?";
    $stmt = $GLOBALS["conn"]->prepare($sql);
    $stmt->bind_param("s", $_POST["vendorName"]);
    $stmt->execute();
